...
|------|---------|---------|---------|
| **金额显示格式** | `amount` 字段返回格式化字符串，收入带负号（如 "-$10.00"），支出不带负号（如 "$10.00"） | 收入显示为绿色正数（如 "$10.00"），支出显示为默认色负数（如 "-$10.00"），转账显示为 "+/-" 格式 | 界面对非转账交易的金额取反后格式化，并根据分类应用颜色样式 |
| **金额符号语义** | 负号表示收入，正号（无符号）表示支出 | 正数表示收入（绿色），负数表示支出（默认色） | 符号规则完全相反，对接方需特别注意 |
...
3. **金额为字符串类型**：`amount` 字段返回格式化字符串（如 "-$10.00"）而非数值类型，调用方需自行解析才能进行数值计算
4. **缺少原始数值字段**：未提供 `amount_value` 类的数值类型字段，对接不便
5. **缺少排除字段过滤**：无法过滤掉 excluded 标记的交易
6. **排序不可配置**：固定按日期倒序，无法自定义排序字段
...
3. **增加数值型金额字段**：添加 `amount_value` 字段返回数值类型的金额（如 `-10.00`），便于调用方直接进行数值计算
4. **支持 excluded 过滤**：增加 `include_excluded` 参数，默认不返回 excluded 交易
5. **可配置排序**：支持 `sort` 和 `order` 参数，允许按 amount、date 等字段排序
6. **明确金额符号规则**：在 API 文档中明确说明：负号表示收入（income），正号（无符号）表示支出（expense）
