...
- **Trade** - 交易记录**Entry** - 账务条目（交易的容器）
- **Holding** - 持仓快照（按日期的持仓）
- **Account** - 账户（聚合根）
- **Balance** - 账户余额快照

```
表单提交 → 控制器 → CreateForm → Entry + Trade 构造 → 异步同步 → 持仓计算 → 余额刷新
```
...
def create_trade
  ...
  if trade_entry.save
    trade_entry.lock_saved_attributes!  # 锁定已保存属性（用于变更追踪）
...
- **利息收入**: 创建普通 `Transaction` 类型的 Entry
- **存款/取款**: 如果提供对方账户则创建 `Transfer`，否则创建普通 `Transaction`
...
同步触发 (sync_later)
    ↓
PortfolioCache (预加载所有交易+价格)
    ↓
ForwardCalculator / ReverseCalculator (逐日计算)
    ↓
Holding (每日持仓快照)
    ↓
Balance (每日余额快照)
...
2. **事件溯源思想**: 交易是不可变事件，持仓是事件投影

3. **策略模式**: Forward/Reverse 两种计算策略

4. **物化视图**: Holding 是 Trade 逻辑物化到数据库

5. **缓存前置**: PortfolioCache 预加载所有依赖数据
...
| 正向计算 | `app/models/holding/forward_calculator.rb` |
| 反向计算 | `app/models/holding/reverse_calculator.rb` |
